...
| ⭐ **[ashgrove](https://github.com/NeilP211/ashgrove)** | A genuinely tense browser **survival-horror game** in **Three.js**: no engine, no build step. Everything is procedural: canvas-noise rot/grime/wallpaper materials, a full post-fx grade (selective bloom, animated film grain, vignette, chromatic aberration), and a fully synthesized **Web Audio** soundscape (HRTF-spatialized enemy voices, heartbeat, sub drone). One fully realized mansion floor with four distinct AI threats (including an **unkillable hunter**) and a locked-key backtrack through a changed house. | `Three.js` `WebGL` `Web Audio` `JavaScript` |
