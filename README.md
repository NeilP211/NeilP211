...
| **[exfil](https://github.com/NeilP211/exfil)** | Browser-playable authoritative-server extraction shooter with competitive netcode (**64Hz tick**, client prediction, lag compensation) **plus self-play RL**: PPO agents trained on the bit-exact sim hit a **100% win rate** vs scripted/random baselines. | `Go` `Rust/WASM` `TypeScript` `PyTorch` `RL` |
