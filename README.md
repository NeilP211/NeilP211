...
| **[yapback](https://github.com/NeilP211/yapback)** ![new](https://img.shields.io/badge/new-2d3b2a?style=flat-square) | A native **macOS app** that catches your friend up on the group chat they ignored: reads iMessage **chat.db** directly (typedstream decoding, AddressBook name resolution), finds the gap since their last message (or a hand-picked "summarize after this" anchor), has **Claude** write the recap in gc voice, and sends it back into the chat via Messages AppleScript with **db-verified delivery**. CSRF-guarded localhost server, stable code-signing identity so the TCC grant survives rebuilds. | `Swift` `AppKit` `Python` `Claude` `AppleScript` |
| **[wildbound](https://github.com/NeilP211/wildbound)** | A magazine-grade **nature-travel** site ([live](https://neilp211.github.io/wildbound/)): 19 destinations + 5 itineraries with editorial photography sourced and **license-verified from Wikimedia Commons**, plus a zero-backend **planning hub** that deep-links pre-filled flight and hotel searches from your saved home airport. Static, responsive, **WCAG-audited**. | `Astro` `Tailwind` `a11y` |
