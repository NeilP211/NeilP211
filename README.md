...
| ⭐ **[Five Nights at Wahmirs](https://github.com/NeilP211/ashgrove)** | A browser **survival-horror game** in **Three.js**: no engine, no build step, everything procedural (canvas-noise materials, a synthesized **Web Audio** soundscape with a unique signature per creature, a full cold post-fx grade). Five named AI threats including an **unkillable hunter**, plus a **diegetic phone** pillar (a live radar you raise at the cost of watching the room) that one creature sabotages and another can only be tracked through, a warm-light ally, and a **5-night escape** with chained objectives and a life system. | `Three.js` `WebGL` `Web Audio` `JavaScript` |
